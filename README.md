...
- Both trained checkpoints receive byte-identical completion prompts; no chat template is used in the released run.
...
1. The released comparison uses a common raw-completion prompt. This controls prompt content, but it also means the Instruct checkpoint is not evaluated in its preferred chat-template format.
