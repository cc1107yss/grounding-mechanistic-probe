...
> 可直接供导师审阅的“推理半边”实验总结。本实验检验冻结模型的注意力中能否解码证明相关性与证明步骤信息，以及指令微调和原生 chat 格式是否改变这些表征；它尚未直接测量符号接地。

## 执行摘要

1. **模型中存在学习得到的推理结构信息。** 在严格的 theory-held-out 切分下，Qwen2.5-7B Base 和 Instruct 的 SP1/SP2 kNN macro-F1 分别为 `0.7184/0.8889` 和 `0.7122/0.8677`，明显高于同架构随机权重对照的 `0.4980/0.5992`。
2. **指令微调没有产生一个清晰分离的新推理阶段。** Base 与 Instruct 的层曲线非常接近。Base 在 SP1 上略高，但 SP2 在 kNN 与线性探针之间出现方向反转；这更像表征几何被重新组织，而不是整体增强或削弱。
3. **原生 chat 格式对行为的影响大于对可解码性的影响。** 对同一个 Instruct checkpoint，raw→chat 的 SP1、SP2 kNN 差异在统计上均与 0 相容；线性探针略偏向 chat，但端任务准确率从 `76.10%` 降至 `74.40%`。
4. **这些结果是推理侧表征证据，不是 grounding–reasoning 已分离的证据。** SP1 测量证明相关性选择，而非实体/谓词绑定；探针成功是观察性结果，不能证明模型因果地使用了这些信息。

## 研究问题与实验边界

长期研究问题是：语言模型的符号接地步骤能否与符号推理步骤分离。本实验通过 ProofWriter 的证明树定义并建立其中的**推理半边**：
